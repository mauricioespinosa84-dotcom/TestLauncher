...
class Splash {
    ...
    async checkUpdate() {
        ...
        ipcRenderer.invoke('update-app').then().catch(err => {
            console.error(err);
            this.setStatus(`Error al buscar actualizacion.<br>Continuando...`);
        ...
        ipcRenderer.on('error', (event, err) => {
            if (!err) return;
            console.error(err);
            this.setStatus(`Error al descargar actualizacion.<br>Continuando...`);
            return this.maintenanceCheck();
        })
        ...
        ipcRenderer.on('update-not-available', () => {
            console.error("No hay actualizacion disponible");
    ...
    async dowloadUpdate() {
        ...
        this.setStatus(`Actualizacion disponible!<br><div class="download-update">Descargar</div>`);
    ...
    async maintenanceCheck() {
        config.GetConfig().then(res => {
            ...
            console.error(e);
            return this.shutdown("No se detecto conexion a internet,<br>intenta de nuevo mas tarde.");
        })
    }
