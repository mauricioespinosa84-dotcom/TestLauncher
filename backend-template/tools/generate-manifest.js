...
function walk(dir, root, out, ignoreSet) {
    const entries = fs.readdirSync(dir, { withFileTypes: true });
    for (const entry of entries) {
        const full = path.join(dir, entry.name);
        if (entry.isDirectory()) {
            walk(full, root, out, ignoreSet);
            continue;
        }
        if (ignoreSet && ignoreSet.has(path.resolve(full))) continue;
...
function main() {
    ...
    const files = [];
    walk(absFolder, absFolder, files, new Set([absOutput]));
